 ...
 *
 * Select adds one to the selected habit. That is deliberately not Habitify's own
 * "mark complete" endpoint, which fills the whole goal at once: the point of
 * showing 1/3 is being able to log the second and the third separately. The press
 * only touches the cache, so it works with the radio off and the number moves
 * immediately; the accumulated amount is pushed on the next sync, one request per
 * habit however many times Select was pressed, before the journal is re-fetched -
 * the same arrangement as Todoist completions.
 *
